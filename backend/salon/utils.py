...
# SMS Provider Abstraction Hierarchy
class BaseSMSProvider:
    def send_sms(self, phone: str, otp: str) -> tuple[bool, str]:
...
class TwilioSMSProvider(BaseSMSProvider):
    def send_sms(self, phone: str, otp: str) -> tuple[bool, str]:
        ...
        from_phone = getattr(settings, 'TWILIO_PHONE_NUMBER', '').strip()

        if not sid or not token or not from_phone:
            return False, "Phone SMS verification is currently unavailable. Please select Email OTP or contact the administrator."

        try:
            from twilio.rest import Client
            client = Client(sid, token)
            client.messages.create(
                body=f"[Glamora] Your account verification code is: {otp}. Valid for 5 minutes.",
                from_=from_phone,
                to=phone
            )
            logger.info(f"[TWILIO SMS SUCCESS] Sent to {phone}")
            return True, "SMS sent successfully via Twilio."
        except Exception as e:
            logger.error(f"[TWILIO SMS ERROR] Phone: {phone} | Error: {str(e)}")
            return False, f"Twilio SMS delivery failed: {str(e)}"
...
class MSG91SMSProvider(BaseSMSProvider):
    def send_sms(self, phone: str, otp: str) -> tuple[bool, str]:
        ...
        if not auth_key:
            return False, "MSG91 credentials missing. Please set MSG91_AUTH_KEY in settings."
        
        try:
            import urllib.request
            import json
            template_id = getattr(settings, 'MSG91_TEMPLATE_ID', '').strip()
            url = f"https://control.msg91.com/api/v5/otp?template_id={template_id}&mobile={phone}&otp={otp}"
            req = urllib.request.Request(url, headers={'authkey': auth_key, 'Content-Type': 'application/json'})
            with urllib.request.urlopen(req) as resp:
                res_data = json.loads(resp.read().decode())
                if res_data.get('type') == 'success':
                    logger.info(f"[MSG91 SMS SUCCESS] Sent to {phone}")
                    return True, "SMS sent successfully via MSG91."
                return False, f"MSG91 SMS error: {res_data.get('message')}"
        except Exception as e:
            logger.error(f"[MSG91 SMS ERROR] Phone: {phone} | Error: {str(e)}")
            return False, f"MSG91 SMS delivery failed: {str(e)}"


class FirebaseSMSProvider(BaseSMSProvider):
    def send_sms(self, phone: str, otp: str) -> tuple[bool, str]:
        key = getattr(settings, 'FIREBASE_SMS_KEY', '').strip()
        if not key:
            return False, "Firebase SMS credentials missing. Please set FIREBASE_SMS_KEY in settings."
        return False, "Firebase SMS provider is not yet configured."


def send_sms_otp(phone: str, otp: str) -> tuple[bool, str]:
    """
    Dispatches SMS OTP via configured SMS Provider (Twilio / MSG91 / Firebase).
    If no provider credentials are configured, returns an explicit error message.
    """
    ...
    providers = {
        ...
        'firebase': FirebaseSMSProvider(),
    }
    
    provider = providers.get(provider_name)
    if not provider:
        return False, f"Unsupported SMS provider: '{provider_name}'."

    logger.info(f"[SMS OTP INITIATED] Target: {phone} | Provider: {provider_name}")
    success, msg = provider.send_sms(phone, otp)
    
    if not success:
        logger.warning(f"[SMS OTP DELIVERY FAILED] Target: {phone} | Reason: {msg}")
        print(f"\n[GLAMORA SMS WARNING] Target: {phone} | Reason: {msg}\n")
    else:
        print(f"\n[GLAMORA SMS DELIVERED] To: {phone} | OTP: {otp}\n")
